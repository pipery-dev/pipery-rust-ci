...
2. Configure Protected Variables in Bitbucket (Repository Settings > Pipelines > Repository Variables):
   ...
   - `GITHUB_TOKEN` - GitHub API access (for reintegration and releases)
3. Commit and push to trigger the pipeline

### Pipeline Stages

The Bitbucket Pipelines equivalent follows the same structure as the GitHub Actions:
- checkout → setup → SAST (cargo-geiger, Semgrep) → SCA (cargo audit) → lint (cargo clippy) → build → test → versioning → packaging → release → reintegration → logs
...
- Same security scanning tools as GitHub Actions (cargo-geiger, Semgrep, cargo audit, cargo clippy)
- Parallel SAST and SCA stages
- Supports stable and nightly Rust toolchains
- Automatic versioning and tagging
- Cross-compile binaries
- crates.io publish support
